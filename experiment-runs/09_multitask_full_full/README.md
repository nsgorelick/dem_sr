...
This run directory is the canonical Plan 09 multitask full/full configuration:
...
```bash
python3 train_experiment.py --config experiment-runs/09_multitask_full_full/run_config.json
```

## 3) Run evaluation

```bash
python3 eval_experiment.py --config experiment-runs/09_multitask_full_full/run_config.json
```

Since `eval.output_json` is `null`, the standardized eval result JSON is written under:

- `experiment-runs/09_multitask_full_full/results/`
