...
class MixtureSpecialistsExperiment(Experiment):
    ...
    def train_epoch_log_suffix(self, metrics: dict[str, float]) -> str:
        ...
        parts.extend(f"{k}={metrics[k]:.3f}" for k in expert_keys)
        if "expert_entropy" in metrics:
            parts.append(f"expert_entropy={metrics['expert_entropy']:.3f}")
        if "expert_divergence" in metrics:
            parts.append(f"expert_divergence={metrics['expert_divergence']:.4f}")
